  ...
  \begin{siglas}
    ...
    \item[SDK] Android software development kit
  \end{siglas}
  % ---
  ...
  % ---
      ...
      O desenvolvimento de aplicações mobile, sempre foi uma área que existiu desde o início dos
      primeiros dispositivos móveis, mas que ganhou mais destaque nos últimos anos. Nas primeiras
      gerações de celulares com interface gráficas e com capacidade de processamento mais robusta,
      uma séries de aplicativos era disponibilizada ao usuário como: agenda, calendário, email,
      jogos e até mesmo acesso a internet. Porém, por mais variadas que as aplicações fossem,
      o usuário não podia instalar aplicativos de terceiros, pelo menos não de uma maneira que fosse simples para um usuário mais leigo. Geralmente o processo envolvia baixar conteúdo de
      desenvolvedores no computador e fazer o deploy manualmente.

      Outro problema que existia, até então, é que o mercado de celulares possuia uma vasta gama
      de sistemas operacionais disponíveis. Usualmente cada companhia desenvolvia seu próprio
      sistema operacional para suas respectivas plataformas, o que eventualmente era um problema
      para os desenvolvedores, visto que sempre que quisessem que algum celular
      acompanhasse seus aplicativos precisavam recompilar o código para a nova plataforma
      e fazer possíveis refatorações nas chamadas de sistema, caso o sistema operacional não
      desse suporte a uma dada chamada a qual estivesse sendo utilizada no código original.

      Com o passar dos anos, a indústria de dispositivos mobile passou por um processo muito
      semelhante ao que houve no mundo de computadores desktop, tanto no quisito hardware quanto
      software. Os componentes eletrônicos para essas plataformas foram ficando cada vez mais
      poderosos e baratos ao longo dos anos. Já no que se refere aos sistemas operacionais, as
      empresas viram que não era viável ter que sempre refatorar um aplicativo para lançar em
      uma nova plataforma. Dessa forma, surgiram sistemas operacionais que hoje, praticamente,
      dominam totalmente o mercado de dispositivos móveis. São esses: Android, iOS e Microsoft
      Phone, produzidos respectivamente pela Google, Apple e Microsoft.
      ...
      Android, seguida de uma explicação, em mais alto nível, de como funciona uma aplicação nesse
      tipo de plataforma. Por último será apresentado, usando um caso real, de como funciona realmente
      uma aplicação dentro Android, apresentando uma modelagem clássica de MVC e como cada um
      desses componentes é descrito, via código, na aplicação.

      \section{Especificações técnicas do Android}
        O Android foi uma plataforma que teve seu primeiron lançamento, oficial, em 2008. Na época,
        ...
        eram CPU's com baixa frequência, apesar de usar a mesma licença da GNU. A biblioteca ficou conhecida como \textit{Bionic}.
        ...
        o MIPS foram arquiteturas que foram ganhar suporte somente mais tarde, visto essas eram
        ...
        \begin{figure}[htbp]
            \begin{center}
                \includegraphics[width=1.0\textwidth]{img/cpusPercentage.png}
            \end{center}
        \caption{\label{fig:passaro}Porcentagem de ISA's que atualmente estão executando Android.}
        \end{figure}


        No que se refere a execução de processos dentro do o sistema operacional utiliza
        o Dalvik, uma máquina virtual de processos, que recompila o Java bytecode para rodar nativamente. Na versão 4.4, o Android introduziu o ambiente \textit{Android Runtime}
        ...
        a mais tempo sem serem utilizaods.

        Já quanto as aplicações que são desenvolvidas para o Android, estas utilizam como suporte
        o SDK (\textit{Android software development kit}). O SDK possui ferramentas como: debugger,
        bibliotecas (API's), emulador baseado no QEMU, documentação. O SDK possui suporte nativo
        a linguagem Java que pode ser combinada com C/C++ e, mais recentemente, a Google anunciou
        que a linguagem Kotlin também passou a ser suportada pelo SDK. Outra \textit{feature}
        interessante do SDK é que ele permite que os desenolvedores testem seus aplicativos
        em versões mais antigas do Android, afim de que possam analisar o desempenho de suas
        aplicações em plataformas mais antigas.

      \section{Desenvolvimento de aplicações mobile}
      \section{Desenvolvimento de aplicações mobile, usando um caso real}
